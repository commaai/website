...
Check your car's compatibility, learn what openpilot does, and choose the hardware you need.
...
In most cars, ACC and LKAS are separate factory features. ACC handles acceleration and braking while the driver steers. LKAS helps with steering, but may only work above certain speeds or when lane lines are clearly visible. The driver turns each feature on and remains responsible for driving.

On compatible cars, openpilot uses the car's existing acceleration, braking, and steering controls to provide one driver assistance system. It combines the jobs of ACC and LKAS to maintain a safe following distance and keep the car centered in its lane.
